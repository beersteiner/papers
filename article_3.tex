...
\affiliation{\textsuperscript{3}\textit{Computer Science, School of Informatics, Computing, and Engineering, Indiana University, Bloomington, IN, USA}} % Author affiliation
...
HOG with XGBoost - When we used Histogram of Oriented Gradients with XGBoost, we are able to achieve \textbf{0.5986} score after submitting the predictions on test data to Kaggle. Figure 9 shows the ROC plot for the prediction made on validation data. Area Under Curve (AUC) for the model is \textbf{0.8170044}. Log-loss for the trained model is \textbf{0.6610249}.\\

\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{iceberg/xgboost.png}
	\caption{HOG with XGBoost ROC plot}\label{roc}
	\small
	True-Positive rate represents icebergs correctly classified as icebergs, and False-Positive rate represents non-icebergs incorrectly classified as icebergs.
\end{figure}

HOG with Random Forest - When we used Histogram of Oriented Gradients with Random Forest, we are able to achieve \textbf{0.5345} score after submitting the predictions on test data to Kaggle. Figure 10 shows the ROC plot for the prediction made on validation data. Area Under Curve (AUC) for the model is \textbf{0.7778883}. Log-loss for the trained model is \textbf{0.5769474}.\\

\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{iceberg/random_forest.png}
	\caption{HOG with Random Forest ROC plot}\label{roc}
	\small
	True-Positive rate represents icebergs correctly classified as icebergs, and False-Positive rate represents non-icebergs incorrectly classified as icebergs.
\end{figure}
